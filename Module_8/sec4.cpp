   ...
   //
   // PURPOSE:
   //   Driver for dynarr3 exercise.
   //   Illustrate exceptions hierarchies and
   //   data passed within exceptions.
...
class SubscriptError // Exception class.
{
    public:
        SubscriptError (char * array, long subscr ){ 
            strcpy(array_name,array);
            bad_subscript = subscr; 
        };
    ...
    size_t array_size;     // Size of the
                           // dynamic array.
    char   array_name[80]; // For educational
                           // purposes only.
    int   *data;           // Pointer to dynamic
   ...
   DynamicIntArray::DynamicIntArray 
    ( size_t size, const char * const name )
   {
   ...
   DynamicIntArray::~DynamicIntArray(void)
   {
    delete data;
    array_size = 0;
    cout << "Destructor called for " << array_name
         << endl;
   }

   
    // Accessor function which returns a reference
    // to an element of the dynamically-allocated
    // memory.
   
   int& DynamicIntArray::operator[](long  subscript )
   {
    if (subscript < 0)
       throw SubscriptTooSmall
          (this->array_name,subscript);
    else if (subscript > (array_size - 1))
       throw SubscriptTooLarge
          (this->array_name,subscript);
   ...
   { 
    ...
    for (i=1; i<=2; ++i)
    {
       ...
       catch (DynamicIntArray::SubscriptTooLarge& 
              theException)
       {
          cout << "*** Error accessing array " 
               << theException.array_name << endl;
          MODIFICATION: ArrayName()
          cout << "*** Subscript " 
               << theException.bad_subscript << 
                  " is too large.\n";
       }
